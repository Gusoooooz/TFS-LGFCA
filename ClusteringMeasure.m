...
% purity
% % 预测类和真实类的差异个数：关心分类正确的数量
correnum = 0;
for ci = 1:pred_classnum
    incluster = Y(find(predY == predLidx(ci))); % 取预测的结果中的一类 对应的 真实标签的类别
%     cnub = unique(incluster);
%     inclunub = 0;
%     for cnubi = 1:length(cnub)
%         inclunub(cnubi) = length(find(incluster == cnub(cnubi)));
%     end;
    inclunub = hist(incluster, 1:max(incluster));  % 此时的 真实标签可能有很多类
    if isempty(inclunub) inclunub=0;end; % incluster 是空集
    correnum = correnum + max(inclunub); % 最多的类 是正确数量 
end;
